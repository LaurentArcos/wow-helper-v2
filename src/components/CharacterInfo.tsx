    ...
    <div className="bg-white p-4 rounded shadow space-y-3 text-sm text-center relative flex flex-col items-center">
      ...
      {character.faction && (
        <img
          ...
          className="absolute top-4 left-4 w-13 h-15 opacity-80"
        />
      )}
      ...
      {character.active_title && (
        <h2 className="text-xl font-semibold italic text-gray-700 capitalize">{character.active_title}</h2>
      )}
      <div>
        <p className="text-lg font-semibold">{character.race} {character.gender}</p>
        <span className="text-s text-gray-600">{character.class} {character.spec}</span>
      </div>
      ...
      <div className="text-left space-y-1 pt-1">
        {character.current_exact_position && (
          <p><strong>Position actuelle :</strong> {character.current_exact_position} ({character.current_exact_region})</p>
        )}
        {character.hearthstone_bind_position && (
          <p><strong>Pierre de foyer :</strong> {character.hearthstone_bind_position} ({character.hearthstone_bind_map})</p>
        )}
        ...
        <p className="flex justify-center text-xs mt-4 text-gray-600 capitalize">Royaume {character.realm}</p>
      </div>
    </div>
  );
}
